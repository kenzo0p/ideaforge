...
export async function buildDocxBuffer(input: DocInput): Promise<Buffer> {
  ...
  children.push(
    ...
    new Paragraph({
      spacing: { after: 400 },
      children: [
        new TextRun({ text: `Generated by Scrutan · ${date}`, italics: true, size: 18, color: MUTED }),
      ],
    }),
  );
  ...
  children.push(
    new Paragraph({
      ...
      children: [
        new TextRun({
          text: "Assembled automatically by Scrutan. Verify sources before relying on them.",
          italics: true,
          size: 17,
          color: MUTED,
        }),
